    ...
    def self.extended(base) #:nodoc:
      base.class_eval do
        ...
        def request(req, body = nil, &block)
          ...
          existing_fake[1][scope][:body] << {:body => response.body.to_s} and return response if existing_fake
          
          @@fakes << [path, {scope => {:method => req.method, :body => [{:body => response.body.to_s}]}}]
